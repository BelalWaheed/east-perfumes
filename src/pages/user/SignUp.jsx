...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/\S+@\S+\.\S+/.test(form.email)) return Swal.fire({ icon: 'error', text: t('auth.invalidEmailFormat') });
    if (!form.gender) return Swal.fire({ icon: 'error', text: t('auth.selectGender') });
    ...
    if (exists) return Swal.fire({ icon: 'error', text: t('auth.emailAlreadyRegistered') });
      ...
      <div className="w-full max-w-md">
        <div className="card-premium p-8 sm:p-10 glass">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-foreground mb-2">{t('common.signUp')}</h1>
            <p className="text-muted-foreground">{t('auth.niceToMeet')}</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="text-sm font-medium text-foreground mb-2 block">{t('auth.name')}</label>
              <input
                name="name"
                value={form.name}
            ...
            <div>
              <label className="text-sm font-medium text-foreground mb-2 block">{t('auth.yourGender')}</label>
              <select
                name="gender"
                value={form.gender}
                onChange={handleChange}
