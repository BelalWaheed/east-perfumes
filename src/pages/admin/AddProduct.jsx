...
const EMPTY = { name: '', description: '', price: '', discount: '0', image: '', category: '', nfcCode: '' };

export default function AddProduct() {
  useSEO({ title: 'Add Product' });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      Swal.fire({ title: 'Product Added!', icon: 'success', timer: 1500, showConfirmButton: false });
  ...
  const fields = [
    { label: 'Product Name *', name: 'name', type: 'text', placeholder: 'e.g. Rose Oud 50ml' },
    { label: 'Image URL *', name: 'image', type: 'url', placeholder: 'https://…' },
    { label: 'Category *', name: 'category', type: 'text', placeholder: 'e.g. oud, musk, rose' },
      ...
      <div className="container" style={{ maxWidth: 700 }}>
        ...
        <div className="card" style={{ padding: '2rem' }}>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.5rem', marginBottom: '1.5rem' }}>
            Add <span className="gradient-gold">Product</span>
          </h1>
          <form onSubmit={submit} noValidate>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
              {fields.map(({ label, name, type, placeholder }) => (
                <div className="form-group" key={name} style={{ margin: 0 }}>
                  ...
                  <input id={`add-${name}`} name={name} type={type} className="input" placeholder={placeholder} value={form[name]} onChange={handle} min={type === 'number' ? 0 : undefined} />
                </div>
              ))}
            </div>
            <div className="form-group" style={{ marginTop: '1rem' }}>
              <label className="label">Description</label>
              <textarea id="add-description" name="description" className="input" rows={4} placeholder="Product description…" value={form.description} onChange={handle} style={{ resize: 'vertical' }} />
            </div>
            {form.image && (
            ...
            <button id="add-product-btn" type="submit" className="btn btn-gold btn-full btn-lg" disabled={loading}>
